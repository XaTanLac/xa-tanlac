...
import { IPublicInfo, useArticleDetailMemo } from '@/hooks/use-article'
import DefaultHeader from '@/layouts/default-header'
import { formatDateDDMMYYYYhhmm } from '@/utils/string-handler'

const PublicArticleDetailPage: React.FC = () => {
  const { openSnackbar } = useSnackbar()
  const { id } = useParams<{ id: string }>()
  const [data, setData] = useState<IPublicInfo>()
  const [publicInfoDetailData, loading] = useArticleDetailMemo<IPublicInfo>(PUBLIC_INFO_API.DETAIL(id || ''), {}, [])
  ...
  useEffect(() => {
    if (!loading && publicInfoDetailData) {
      setData(publicInfoDetailData)
    }
