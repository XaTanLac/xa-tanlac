...
import { useFoodTravelDetailMemo } from '@/hooks/use-food-travel'
import { IArticle } from '@/hooks/use-news'
import DefaultHeader from '@/layouts/default-header'
import { formatDateDDMMYYYYhhmm } from '@/utils/string-handler'

const FoodTravelDetailPage: React.FC = () => {
  const { id } = useParams<{ id: string }>()
  const [data, setData] = useState<IArticle>()
  const [foodTravelDetailData, loading] = useFoodTravelDetailMemo(id || '', {}, [])

  useEffect(() => {
    if (!loading && foodTravelDetailData) setData(foodTravelDetailData)
  }, [foodTravelDetailData, loading])
...
export default FoodTravelDetailPage
